      ...
      <div className="relative">
        ...
        {education.map((edu, index) => (
          ...
          >
            ...
            {/* Content Section */}
            <div
              className={`w-full sm:max-w-md lg:max-w-md p-2 sm:p-4 lg:p-4 rounded-2xl shadow-2xl border border-white bg-gray-900 backdrop-blur-md shadow-[0_0_20px_1px_rgba(130,69,236,0.3)] ${
                index % 2 === 0 ? "sm:ml-0 lg:ml-0" : "sm:mr-0 lg:mr-0"
              } sm:ml-16 sm:mr-16 ml-6 lg:ml-2 lg:mr-2 transform transition-transform duration-300 hover:scale-105`}
            >
              <div className="flex items-center space-x-2 sm:space-x-4 lg:space-x-3">
                {/* School Logo/Image */}
                <div className="w-10 h-8 sm:w-16 sm:h-12 lg:w-10 lg:h-8 bg-white rounded-md overflow-hidden flex-shrink-0">
                  <img
                    src={edu.img}
                    alt={edu.school}
              ...
              <p className="mt-2 text-xs sm:text-responsive-base lg:text-xs text-gray-400 font-bold">Grade: {edu.grade}</p>
              <p className="mt-2 text-xs sm:text-responsive-base lg:text-xs text-gray-400 leading-relaxed">{edu.desc}</p>
            </div>
          </div>
        ))}
      </div>
